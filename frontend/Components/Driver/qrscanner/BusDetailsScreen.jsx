// Components/Driver/qrscanner/BusDetailsScreen.jsx
...
export default function BusDetailsScreen({ route, navigation }) {
  const { busData, driverUniqueId, driver } = route.params; // 👈 also get driver
  ...
  const onProceed = async () => {
    ...
    if (result.success) {
      navigation.navigate("DriverDashboard", {
        driverUniqueId,
        busData,
        driver, // 👈 pass driver so Dashboard shows real name
      });
    } else {
    ...
    <View style={styles.container}>
      ...
      <Pressable
        ...
        onPress={() => navigation.replace("QRScanner", {
          driverUniqueId,
          driver, // 👈 pass back so QRScanner still has it
        })}
      >
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 24, fontWeight: "bold", textAlign: "center", marginBottom: 30, marginTop: 20 },
  label: { fontSize: 16, marginBottom: 5, color: "#2C2C7C" },
  input: { borderWidth: 1, borderRadius: 10, padding: 12, marginBottom: 15, backgroundColor: "#F2F2F2" },
  button: { marginTop: 30, backgroundColor: "#4834D4", padding: 15, borderRadius: 30, alignItems: "center" },
